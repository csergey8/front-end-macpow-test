...
const handlers: any = {
  ...
  [jokesActions.ADD_JOKE_TO_FAVOURITE]: (state: JokeState, action: any) => {
    if((state.favouriteJokes as JokeType[]).map((joke) => joke.id).includes(action.payload.id)){
      return state
    }
  ...
  [jokesActions.REMOVE_JOKE_FROM_FAVOURITE]: (state: JokeState, action: any) => {
    const favouriteJokes = state.favouriteJokes.filter((joke) => joke.id !== action.id)
    localStorage.setItem('JOKES_FAVOURITE', JSON.stringify(favouriteJokes))
    return {
      ...state,
      favouriteJokes
